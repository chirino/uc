...
func run(forceDownload bool) error {
    ...
    catalog := &cmd.CatalogConfig{}
    err := loadYaml(catalogFileName, catalog)
    if err != nil {
        return err
    }

    // findNewKubectlReleases(catalog)
    for command, _ := range catalog.Commands {
...
func signIfNeeded(fn string) error {
    err := cmd.CheckSigatureAgainstSigFile(fn)
    if err != nil {
        err = sign(fn)
        if err != nil {
            return err
...
func loadYaml(fileName string, catalog interface{}) error {
...
func findNewKubectlReleases(catalog *cmd.CatalogConfig, forceDownload bool) error {
    command := "kubectl"
minor:
    for minor := 15; ; minor++ {
    micro:
...
func storeYaml(catalog string, config interface{}) error {
    bytes, err := yaml.Marshal(config)
    if err != nil {
        return err
    }
    fmt.Println("storing catalog")
    ioutil.WriteFile(catalog, bytes, 0755)
    if err != nil {
        return err
    }
    return nil
